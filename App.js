...
import { StatusBar, StyleSheet, View, Text, Button, TextInput, TouchableOpacity, FlatList, Modal } from 'react-native';
...
export default function App() {
  ...
  const handleAddEvent = () => {   
    ...
    setEventList((prevEvent) => [
      ...eventList, {id: Math.random().toString(), value: inputTxt}
    ]);
    setInputTxt('');
  }
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.texto}>
        {
          eventList.length === 1 ? `Tengo ${eventList.length} evento programado` : `Tengo ${eventList.length} eventos programados`
        }        
      </Text>            
      ...
      <View style={styles.listaContainer}>                
        ...
        <FlatList          
          data={eventList}
          renderItem={ ({item}) => (
            <TouchableOpacity style={styles.eventItem} onPress={() => onPressItem(item.id)}>
